  ...
  window.onload = function(){
    ...
    setInterval(function(){
      ...
      <div class="d-flex justify-content-between">
        ...
        <a href="{{route('adresa.form')}}" class='btn btn-lg btn-info my-4'>Adauga contract</a>
      ...
      <table class="table table-sm px-0 " id='entry-table'>
          <thead>
              ...
              <th scope="col">Nr. contract</th>
                  <th scope="col">Data inceper</th>
                  <th scope="col">Data inchiere</th>
                  <th scope="col">Pret lunar</th>
          ...
          @foreach($contracts as $key => $contract)
              ...
              <td>{{$contract-> idcontract}}</td>
                  <td>{{$contract-> inceperecontract}}</td>
                  <td>{{$contract-> sfarsirecontract}}</td>
                  <td>{{$contract-> pretlunar}}</td>
       
                 
                  <td>
                
                <a href="/contractedit/{{$contract->idcontract}} "class='btn btn-md btn-info'>Editeaza</a>
                <a href="/stergerecontract?id={{$contract->idcontract}}"class='btn btn-danger'>Delete</a>
