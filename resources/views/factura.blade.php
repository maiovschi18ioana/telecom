  ...
  window.onload = function(){
    ...
    setInterval(function(){
      ...
      <div class="d-flex justify-content-between">
        ...
        <a href="{{route('factura.form')}}" class='btn btn-lg btn-info my-4'>Adauga factura</a>
      ...
      <table class="table table-sm px-0 " id='entry-table'>
          <thead>
              ...
              <th scope="col">Nr. factura</th>
                  ...
                  <th scope="col">Data generata</th>
                  <th scope="col">Data scadenta</th>
                  <th scope="col">De plata</th>
                  <th scope="col">Stare factura</th>
          ...
          @foreach($facturas as $key => $factura)
              ...
              <td>{{$factura-> idfactura}}</td>
                  ...
                  <td>{{$factura-> facturagenerata}}</td>
                  <td>{{$factura-> facturascadenta}}</td>
                  <td>{{$factura-> plata}}</td>
                  <td>{{$factura-> starefactura}}</td>
          
                  <td>
                
                <a href="/facturaedit/{{$factura->idfactura}} "class='btn btn-md btn-info'>Editeaza</a>
                <a href="/stergerefactura?id={{$factura->idfactura}}"class='btn btn-danger'>Delete</a>
