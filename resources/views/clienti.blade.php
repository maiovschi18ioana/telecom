  ...
  window.onload = function(){
    ...
    setInterval(function(){
      ...
      <div class="d-flex justify-content-between">
        ...
        <a href="{{route('clienti.form')}}" class='btn btn-lg btn-info my-4'>Adauga client</a>
      ...
      <table class="table table-sm px-0 " id='entry-table'>
          <thead>
              ...
              <th scope="col">Nr. client</th>
                  ...
                  <th scope="col">CNP</th>
                  <th scope="col">Stare financiara</th>
          ...
          @foreach($clients as $index=>$client)
              ...
              <td>{{$client-> idclient}}</td>
                  ...
                  <td>{{$client-> cnp}}</td>
                  <td>{{$client-> starefinanciara}}</td>
                  <td>{{$adrese[$index]-> strada.' numar '.$adrese[$index]-> numar.' '.$adrese[$index]-> apartament.', '.$adrese[$index]-> oras}}</td>
                

                  <td>
                
                <a href="/clientiedit/{{$client->idclient}}"class='btn btn-md btn-info'>Editeaza client</a>
                <a href="/adresaedit/{{$adrese[$index]->idadresa}}"class='btn btn-md btn-info'>Editeaza adresa client</a>
                <a href="/stergeclient?id={{$client->idclient}}"class='btn btn-danger'>Delete</a>
